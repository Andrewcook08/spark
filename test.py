...
def generate_player_report(player_data, model, scaler):
    ...
    if is_hitter:
        ...
        ab = int(player_data['AB']) if not pd.isna(player_data['AB']) else 0
    ...
    def generate_report(player_data, model_avg, scaler_avg, model_era, scaler_era, model_war, scaler_war):
        report = ""
    ...
    if all(col in player_data for col in ["OB%", "SLG%", "OPS"]):
        predicted_avg = predict_batting_avg(player_data, model_avg, scaler_avg)
        report += f"🔹 **Predicted AVG (Next Phase):** {predicted_avg}\n\n"

    if "IP" in player_data:
        report += f"🔹 **Current IP:** {float(player_data['IP']):.3f}\n"

    if all(col in player_data for col in ["WHIP", "BB_Pitching", "SO_Pitching", "H_Pitching", "HR_Pitching", "IP", "B/AVG"]):
        predicted_era = predict_era(player_data, model_era, scaler_era)
        report += f"🔹 **Predicted ERA (Next Phase):** {predicted_era}\n\n"
    
    if all(col in player_data for col in ["OPS", "SLG%", "OB%", "GP-GS", "AB", "H", "HR", "RBI", "SB-ATT", "FLD%", "PO", "A", "E", "DP", "IP", "ERA", "WHIP", "SO_Pitching", "BB_Pitching", "H_Pitching", "HR_Pitching", "B/AVG", "APP-GS", "W-L", "SV", "R_Pitching"]):
        predicted_war = predict_war(player_data, model_war, scaler_war)
        report += f"🔹 **Predicted WAR (Next Phase):** {predicted_war}\n\n"

    return report

# Train Prediction Model
def train_prediction_model(df, target, features):
...
# Define target and features BEFORE calling the function
target_column = "AVG"  
...
def predict_batting_avg(player_data, model, scaler):
    return predict_stat(player_data, model, scaler, ["OB%", "SLG%", "OPS"], "AVG")

# Function to predict ERA
def predict_era(player_data, model, scaler):
    return predict_stat(player_data, model, scaler, ["WHIP", "BB_Pitching", "SO_Pitching", "H_Pitching", "HR_Pitching", "IP", "B/AVG"], "ERA")

# Function to predict WAR
def predict_war(player_data, model, scaler):
    if float(player_data.get("IP", 0)) > 0:
        features = ["ERA", "WHIP", "IP", "SO_Pitching", "BB_Pitching", "H_Pitching", "HR_Pitching", "B/AVG", "APP-GS", "W-L", "SV", "R_Pitching"]
    else:
        features = ["OPS", "SLG%", "OB%", "GP-GS", "AB", "H", "HR", "RBI", "SB-ATT", "FLD%", "PO", "A", "E", "DP"]
    return predict_stat(player_data, model, scaler, features, "WAR")

# Generalized function to predict a stat
def predict_stat(player_data, model, scaler, feature_list, stat_name):
    """
    Generalized function to predict a stat based on player data, model, and scaler.
    """
    if model is None or scaler is None:
        return "Prediction unavailable (insufficient data)."

    try:
        features = np.array([[float(player_data[col]) for col in feature_list]])
        features_scaled = scaler.transform(features)
        predicted_value = model.predict(features_scaled)[0]
        
        # Format the prediction based on stat_name
        if stat_name == "AVG":
            return f"{predicted_value:.3f}"  # For batting average, show 3 decimal places
        else:
            return f"{predicted_value:.2f}"  # For other stats like ERA and WAR, show 2 decimal places
    except Exception as e:
        print(f"Prediction error ({stat_name}): {e}")
...
def predict_war(player_data, model, scaler):
    if float(player_data.get("IP", 0)) > 0:
        # Pitching features for WAR prediction
        features = ["ERA", "WHIP", "IP", "SO_Pitching", "BB_Pitching", "H_Pitching", "HR_Pitching", "B/AVG", "APP-GS", "W-L", "SV", "R_Pitching"]
    else:
        # Batting and fielding features for WAR prediction
        features = ["OPS", "SLG%", "OB%", "GP-GS", "AB", "H", "HR", "RBI", "SB-ATT", "FLD%", "PO", "A", "E", "DP"]
    return predict_stat(player_data, model, scaler, features, "WAR")
